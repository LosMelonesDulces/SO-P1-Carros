...
void *cruzar_calle(void *arg)
{
    ...
    if (tiempo_cruce_calculado <= 0)
        tiempo_cruce_calculado = 1;
    ...
    if (configuracion.algoritmo_calendarizacion == RR)
    {
        ...
        if (carro->tiempo_faltante > configuracion.w)
        {
            ...
            tiempo_total = tiempo_cruce_calculado;
            ...
            if (server_is_client_connected()) {
                ...
                sprintf(mensaje, "(%d, %d, %d, %d)\n", carro->lado, carro->id, tiempo_cruce_calculado, (int) 600*tiempo_cruce_calculado/tiempo_total);
...
void *simulacion(void *arg)
{
    ...
    while (!local_fin_simulacion)
    { // Usar variable local para la condición del bucle
        ...
        {
        case EQUIDAD:
            if (direccion_actual_flujo == 0)
            { // Prioritize Izquierda
            ...
            { // Prioritize Derecha (direccion_actual_flujo == 1)
                cethread_mutex_lock(&cola_derecha.mutex);
                if (cola_derecha.cantidad > 0 && carros_pasados_derecha < configuracion.w)
                {
                    carro_actual = cola_derecha.carros[0];
        ...
        if (local_fin_simulacion)
        { // Re-verificar después de la lógica de selección por si se marcó fin
            break;
        }
            ...
            else
            {
                ...
                if (!configuracion.usar_teclado)
                {
                    ...
                    if (todos_han_cruzado_ahora)
                    {
                        ...
                        cethread_mutex_lock(&mutex_fin_simulacion);
                        if (!fin_simulacion)
                        { // Solo establecer si no estaba ya
                            fin_simulacion = true;
        ...
        { // No se procesó ningún carro en esta iteración
            ...
            if (!local_fin_simulacion)
            { // Solo dormir si no estamos a punto de salir
                usleep(100000);
            }
        }
